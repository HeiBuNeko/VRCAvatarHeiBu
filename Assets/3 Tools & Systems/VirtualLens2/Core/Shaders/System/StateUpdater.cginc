...
float _TouchOverride;     // Touch override enable
float _TouchOverrideX;    // Touch override X
float _TouchOverrideY;    // Touch override Y
...
float4 computeTouchingPoint(){
	if((int)round(_TouchOverride)){
		const float2 scale  = INV_SCREEN_ENLARGEMENT;
		const float2 offset = (1.0 - scale) * 0.5;
		const float2 raw = float2(_TouchOverrideX, _TouchOverrideY);
		return float4(1, raw * scale + offset, 1);
	}else{
...
float computeTargetFocusDistance(float old){
	...
	if((int)round(_FocusLock)){
		...
		const float scale = ASPECT * (float)height / (float)width;
		const float offset = (1.0 - scale) * 0.5;
		const float2 uv = computeTouchingPoint().yz;
		const int2 pos = (int2)floor(
			float2(uv.x * scale + offset, uv.y) * float2(width, height));
