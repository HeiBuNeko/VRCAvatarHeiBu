	...
	float  bucket_offset)
{
	...
	const float  mult         = saturate(coc - 0.5);
	const float3 rgb          = fetched.rgb * (mult / max(fetched.a, 1e-3));
...
float4 blurBackground(int num_rings, float2 uv){
	...
	if((tile.y - tile.x) < tile.y * 0.05){
		...
		// Do not use early return to suppress weired warnings
		ret = acc;
	}else{
		...
		if(prev_bucket.weight > 0.0){
			ret = float4(prev_bucket.color, prev_bucket.weight);
		}
	}
	return ret;
}
