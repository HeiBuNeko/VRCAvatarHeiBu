    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct v2f
            {
                ...
                float3 vertexWorldPos : TEXCORD2;
                float3 objectWorldPos : TEXCORD3;
            ...
            v2f vert(appdata v)
            {
                ...
                o.objectWorldPos = unity_ObjectToWorld._m03_m13_m23;
                return o;
            }
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                //頂点のローカル座標
                float3 vertexLocalPos = i.vertexWorldPos - i.objectWorldPos;
                //球の法線
                float3 sphereNormal = normalize(vertexLocalPos);
                //球の半径
                fixed radius = length(vertexLocalPos);
                //スクリーンスペースUV取得
                ...
                //距離フェード
                half dist = length(_WorldSpaceCameraPos.xyz - i.objectWorldPos.xyz);
                half distFade = saturate(dist*_DistFadeTrend - _DistFadeTrend*radius);
                depth = lerp(depth, 1, distFade);

                //リムフェード
                half3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.vertexWorldPos);
                half rimFade = saturate(-dot(viewDirection, sphereNormal));
