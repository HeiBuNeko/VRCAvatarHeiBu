...
Texture2D<float4> _MainTex;        // Display content
...
v2f vertex(appdata v){
	...
	const int flip = ((int)round(_HorizontalFlip) * -2) + 1;
	const int roll = ((int)round(extractRoll() * (2.0 / PI)) * flip) & 3;
	float2 pos = float2(
