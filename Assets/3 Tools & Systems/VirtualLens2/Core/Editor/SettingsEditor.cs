...
namespace VirtualLens2
{
    ...
    public class SettingsEditor : Editor
    {
        ...
        private class GeneralSettingsInspector : SettingsInspectorBase
        {
            ...
            public override bool DrawGUI()
            {
                ...
                else
                {
                    ...
                    {
                        var menuInstaller = SettingsComponent.GetComponent<ModularAvatarMenuInstaller>();
                        if (menuInstaller == null)
                        {
                            EditorGUILayout.HelpBox(
                                "You can add a MA Menu Installer component to customize menu item installation.",
                                MessageType.Info);
                            using (new GUILayout.HorizontalScope())
                            {
                                GUILayout.Space(15.0f);
                                if (GUILayout.Button("Add MA Menu Installer Component"))
                                {
                                    AddMaMenuInstaller(SettingsComponent.gameObject);
                                    needReset = true;
                                }
                            }
                        }
            ...
            {
#if WITH_NDMF && WITH_MODULAR_AVATAR
                // Apply property modifications before migration
                ...
                var menuInstaller = component.GetComponent<ModularAvatarMenuInstaller>();
                if (menuInstaller == null)
                {
                    menuInstaller = Undo.AddComponent<ModularAvatarMenuInstaller>(component.gameObject);
                }
                // VirtualLens2/Core/Expressions/Dummy.asset
                menuInstaller.menuToAppend =
                    AssetUtility.LoadAssetByGUID<VRCExpressionsMenu>("1be79a2afe1bb554dbc56690b57db6dc");
            ...
            private void AddMaMenuInstaller(GameObject target)
            {
#if WITH_NDMF && WITH_MODULAR_AVATAR
                var menuInstaller = Undo.AddComponent<ModularAvatarMenuInstaller>(target);
                // VirtualLens2/Core/Expressions/Dummy.asset
                menuInstaller.menuToAppend =
                    AssetUtility.LoadAssetByGUID<VRCExpressionsMenu>("1be79a2afe1bb554dbc56690b57db6dc");
#endif
        ...
        private class DroneInspector : SettingsInspectorBase
        {
            ...
            private readonly SerializedProperty _enableQuickTurnProp;
            private readonly SerializedProperty _enableZoomProp;
            ...
            public DroneInspector(SerializedObject so) : base(so)
            {
                ...
                _enableQuickTurnProp = so.FindProperty("enableDroneQuickTurn");
                _enableZoomProp = so.FindProperty("enableDroneZoom");
            }
            ...
            public override bool DrawGUI()
            {
                ...
                using (new EditorGUI.DisabledScope(!enabled))
                {
                    ...
                    EditorGUILayout.PropertyField(_enableQuickTurnProp, new GUIContent("Enable Quick Turn"));
                    EditorGUILayout.PropertyField(_enableZoomProp, new GUIContent("Enable Zoom Control"));
                }
                --EditorGUI.indentLevel;
                EditorGUILayout.Space();
                return false;
        ...
        private class DisplaySettingsInspector : SettingsInspectorBase
        {
            ...
            private readonly SerializedProperty _initialGridOpacityProp;
            ...
            public DisplaySettingsInspector(SerializedObject so) : base(so)
            {
                ...
                _initialGridOpacityProp = so.FindProperty("initialGridOpacity");
            ...
            public override bool DrawGUI()
            {
                ...
                EditorGUILayout.Slider(_initialGridOpacityProp, 0.0f, 1.0f, new GUIContent("Grid Opacity"));
        ...
        {
            private readonly SerializedProperty _defaultResoluionProp;
            ...
            {
                _defaultResoluionProp = so.FindProperty("defaultResolution");
            ...
            public override bool DrawGUI()
            {
                ...
                IntPopup(_defaultResoluionProp, resolutions, "Default Resolution");
        ...
        // Localization table
        private LocalizationAsset _localizer;
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                if (!inspector.DrawGUI()) continue;
                // Force reset inspector
                serializedObject.ApplyModifiedProperties();
                return;
            }
            serializedObject.ApplyModifiedProperties();
            ...
            var valid = true;
            var localizationTable = Localization.GetDefaultLocalizationTable();
            foreach (var msg in SettingsValidator.Validate(settings))
            {
                if (msg.Type == MessageType.Error) valid = false;
                var text = string.Format(
                    localizationTable.GetLocalizedString(msg.Key),
                    msg.Substitutions.Select(s => (object)s).ToArray());
                EditorGUILayout.HelpBox(text, msg.Type);
            }
            EditorGUILayout.Space();
