...
namespace VirtualLens2
{
    internal static class SettingsMigrator
    {
        ...
        // Migration for v2.10.2: Add MA Menu Installer automatically
        private static void MigrateMaMenuInstaller(SerializedObject settings)
        {
#if WITH_NDMF && WITH_MODULAR_AVATAR
            if (settings.FindProperty("lastVersion").intValue >= 21002) { return; }
            if (settings.FindProperty("buildMode").enumValueIndex != (int)BuildMode.NonDestructive) { return; }

            // VirtualLens2/Core/Expressions/Dummy.asset
            var dummyMenu = AssetUtility.LoadAssetByGUID<VRCExpressionsMenu>("1be79a2afe1bb554dbc56690b57db6dc");

            var component = (Component)settings.targetObject;
            var obj = component.gameObject;
            var installer = obj.GetComponent<ModularAvatarMenuInstaller>();
            if (installer == null)
            {
                installer = obj.AddComponent<ModularAvatarMenuInstaller>();
            }
            installer.menuToAppend = dummyMenu;
#endif
        }
        
        // Migration for v2.12.0: Convert 4K mode to default resolution
        private static void MigrateDefaultResolution(SerializedObject settings)
        {
            if (settings.FindProperty("lastVersion").intValue >= 21200) { return; }
            
            var legacyProp = settings.FindProperty("useHighResolution");
            var newProp = settings.FindProperty("defaultResolution");
            if (legacyProp.boolValue)
            {
                newProp.enumValueIndex = (int)InternalResolution.Resolution2160p;
            }
            else
            {
                newProp.enumValueIndex = (int)InternalResolution.Resolution1080p;
            }
        }
        ...
        public static void Migrate(SerializedObject settings)
        {
            ...
            MigrateMaMenuInstaller(settings);
            MigrateDefaultResolution(settings);
