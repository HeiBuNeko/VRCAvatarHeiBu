...
namespace VirtualLens2.AV3EditorLib
{
    ...
    {
        private static readonly HashSet<string> AV3BuiltInParameters = new HashSet<string>
        {
            "IsLocal",
            "Viseme",
            "Voice",
        ...
        public static bool IsMergeable(AnimatorController destination, AnimatorController source)
        {
            ...
            foreach (var dst in destination.parameters)
            {
                ...
                {
                    Debug.Log($"Parameter '{dst.name}' exists in both animators with different types.");
                    return false;
                }
                if (AV3BuiltInParameters.Contains(dst.name))
                {
                    // Skip if parameter is a built-in parameter because its default value is useless.
                    continue;
                }
                switch (dst.type)
                {
                    case AnimatorControllerParameterType.Bool:
                        if (src.defaultBool != dst.defaultBool)
                        {
                            Debug.Log($"Parameter '{dst.name}' exists in both animators with different defaults.");
                            return false;
                        }
                        break;
                    case AnimatorControllerParameterType.Float:
                        if (!src.defaultFloat.Equals(dst.defaultFloat))
                        {
                            Debug.Log($"Parameter '{dst.name}' exists in both animators with different defaults.");
                            return false;
                        }
                    ...
                    case AnimatorControllerParameterType.Int:
                        if (src.defaultInt != dst.defaultInt)
                        {
                            Debug.Log($"Parameter '{dst.name}' exists in both animators with different defaults.");
                            return false;
                        }
