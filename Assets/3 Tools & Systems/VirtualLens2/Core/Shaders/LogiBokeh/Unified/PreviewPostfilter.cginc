#ifndef VIRTUALLENS2_LOGIBOKEH_UNIFIED_PREVIEW_POSTFILTER_CGINC
#define VIRTUALLENS2_LOGIBOKEH_UNIFIED_PREVIEW_POSTFILTER_CGINC
...
float4 fragment(v2f i) : SV_Target {
	if(inArea(i.uv, BLURRED_TEXTURE_PREVIEW_BACKGROUND_AREA)){
		...
		return backgroundPostfilter(transformUV(i.uv, BLURRED_TEXTURE_PREVIEW_BACKGROUND_AREA), i.uv);
	}else if(inArea(i.uv, BLURRED_TEXTURE_PREVIEW_FOREGROUND_AREA)){
		// TODO: discard unnecessary pixels
		return foregroundPostfilter(transformUV(i.uv, BLURRED_TEXTURE_PREVIEW_FOREGROUND_AREA), i.uv);
	}
	discard;
