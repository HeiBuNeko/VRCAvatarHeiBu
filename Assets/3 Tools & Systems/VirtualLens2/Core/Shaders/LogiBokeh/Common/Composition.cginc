...
float4 fetchBlurredBackground(float2 uv){
	...
	const float  pixels_scale  = screen_aspect > target_aspect
		? (_ScreenParams.x / TARGET_RESOLUTION.x)
		: (_ScreenParams.y / TARGET_RESOLUTION.y);
	const float2 texel_size    = 2.0 * pixels_scale / CAPTURE_RESOLUTION;
	const float2 uv_pixels     = uv * (pixels_scale * CAPTURE_RESOLUTION);
	const float2 subpix_offset = uv_pixels - floor(uv_pixels);
	const float2 signs         = subpix_offset * 4.0 - 4.0;
	const float2 center_uv     = uv - 0.25 * texel_size * signs;
	const float2 horizontal_uv = center_uv + float2(signs.x, 0.0) * texel_size;
	const float2 vertical_uv   = center_uv + float2(0.0, signs.y) * texel_size;
	const float2 diagonal_uv   = center_uv + signs * texel_size;
	float4 acc = 0.0;
...
float4 gatherFullResolution(float2 uv){
	...
	const float2 texel_size = FULL_COC_TEXEL_SIZE;
