...
namespace RedNightWorks.NadeSystem
{
    public class NadeSystemProcessor : MonoBehaviour
    {
        public static void MainProcess(BuildContext ctx)
        {
            ...
            var footSystem = nadeSystem.gameObject.transform.Find("FootSystem");
            AnimatorController footAnimatorController = null;
            if (footSystem != null)
            {
                footAnimatorController = footSystem.GetComponent<ModularAvatarMergeAnimator>()?.animator as AnimatorController;
                Debug.Log("NadeSystemProcessor.MainProcess: footAnimatorController found");
            }

            var rightHandStates = FindStatesByNamePrefix(animatorController, "RNW/Nade/NadeSoundSelect", "SetRightHandSound ");
            var leftHandStates = FindStatesByNamePrefix(animatorController, "RNW/Nade/NadeSoundSelect", "SetLeftHandSound ");
            var headStates = FindStatesByNamePrefix(animatorController, "RNW/Nade/NaderareSoundSelect", "SetHeadSound ");
            var rightFootStates = footAnimatorController != null
                ? FindStatesByNamePrefix(footAnimatorController, "RNW/Nade/FootNadeSoundSelect", "SetRightFootSound ")
                : null;
            var leftFootStates = footAnimatorController != null
                ? FindStatesByNamePrefix(footAnimatorController, "RNW/Nade/FootNadeSoundSelect", "SetLeftFootSound ")
                : null;
            ...
            if ( rightFootStates != null && rightFootStates.Count == 16)
            {
                AssignAudioClipsToStates(rightFootStates, audioClips);
            }
            if ( leftFootStates != null && leftFootStates.Count == 16)
            {
                AssignAudioClipsToStates(leftFootStates, audioClips);
            }
