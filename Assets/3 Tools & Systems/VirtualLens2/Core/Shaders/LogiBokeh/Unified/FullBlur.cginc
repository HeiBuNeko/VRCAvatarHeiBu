#ifndef VIRTUALLENS2_LOGIBOKEH_UNIFIED_FULL_BLUR_CGINC
#define VIRTUALLENS2_LOGIBOKEH_UNIFIED_FULL_BLUR_CGINC

#include "UnityCG.cginc"
#include "../../Common/Constants.cginc"
#include "../../Common/TargetDetector.cginc"
#include "../../Common/MultiSamplingHelper.cginc"
#include "../Common/Samplers.cginc"

#include "RingKernel.cginc"
#include "../Common/BackgroundBlur.cginc"
#include "../Common/ForegroundBlur.cginc"
#include "Utility.cginc"


//---------------------------------------------------------------------------
// Parameter declarations
//---------------------------------------------------------------------------

float _Blurring;
int _MaxNumRings;


//---------------------------------------------------------------------------
// Structure definitions
//---------------------------------------------------------------------------

struct appdata {
	float4 vertex : POSITION;
	float2 uv     : TEXCOORD0;
};

struct v2f {
	float4 vertex : SV_POSITION;
	float2 uv     : TEXCOORD0;
};


//---------------------------------------------------------------------------
// Vertex shader
//---------------------------------------------------------------------------

v2f vertex(appdata v){
	v2f o;

	if(_Blurring == 0.0 || !isVRChatCameraHighResolution()){
		o.vertex = 0;
		o.uv     = 0;
		return o;
	}

	const float x = v.uv.x;
	const float y = v.uv.y;

	const float proj_x = (x * 2 - 1);
	const float proj_y = (y * 2 - 1) * _ProjectionParams.x;
#ifdef UNITY_REVERSED_Z
	const float proj_z = 1.0;
#else
	const float proj_z = UNITY_NEAR_CLIP_VALUE;
#endif
	o.vertex = float4(proj_x, proj_y, proj_z, 1);
	o.uv     = v.uv;
	return o;
}


//---------------------------------------------------------------------------
// Fragment shader
//---------------------------------------------------------------------------

float4 fragment(v2f i) : SV_Target {
	const int num_rings = (int)(_MaxNumRings * NUM_RINGS_SCALER);
	if(inArea(i.uv, BLURRED_TEXTURE_BACKGROUND_AREA)){
		return blurBackground(num_rings, computePaddedUV(transformUV(i.uv, BLURRED_TEXTURE_BACKGROUND_AREA)));
	}else if(inArea(i.uv, BLURRED_TEXTURE_FOREGROUND_AREA)){
		return blurForeground(num_rings, computePaddedUV(transformUV(i.uv, BLURRED_TEXTURE_FOREGROUND_AREA)));
	}
	discard;
	return 0; // to suppress warnings
}


#endif
