#ifndef VIRTUALLENS2_LOGIBOKEH_UNIFIED_PREFILTER_CGINC
#define VIRTUALLENS2_LOGIBOKEH_UNIFIED_PREFILTER_CGINC
...
v2f vertex(appdata v){
	...
	o.uv     = v.uv;
	o.params = computeCocParameters(_DepthTex, TARGET_RESOLUTION);
	return o;
}
...
float4 fragment(v2f i) : SV_Target {
	if(inArea(i.uv, DOWNSAMPLED_TEXTURE_FULL_AREA)){
		return premultiplyFull(_MainTex, _DepthTex, transformUV(i.uv, DOWNSAMPLED_TEXTURE_FULL_AREA), i.params);
	}else if(inArea(i.uv, DOWNSAMPLED_TEXTURE_BACKGROUND_AREA)){
		return downsampleBackground(_MainTex, _DepthTex, transformUV(i.uv, DOWNSAMPLED_TEXTURE_BACKGROUND_AREA), i.params);
	}else if(inArea(i.uv, DOWNSAMPLED_TEXTURE_FOREGROUND_AREA)){
		return downsampleForeground(_MainTex, _DepthTex, transformUV(i.uv, DOWNSAMPLED_TEXTURE_FOREGROUND_AREA), i.params);
	}
	discard;
